...
public class Matrix {
    ...
    /**
     * матрица на основе строки 1 2 3 4 5 6 2;3;1;2;3;4;5;6
     */
...
//комент !!
    ...
    public boolean isE() {
        ...
        if (isKvadrat()) {
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    if (i == j) {
                        if (!A[i][j].equals(new Q("1"))) {
                            t = 1;
                        }
                    } else if (!A[i][j].equals(new Q("0"))) {
                        t = 1;
                    }
                }
            }
    ...
    /**
     * Количевто строк (элементов в столбце)
     */
    public int getM() {
        return m;
    }
    ...
    /**
     * Количевто столбцов (элементов в строке)
     */
    public int getN() {
        return n;
    }
    ...
    /**
     * Возвращает i,j элемент матрицы i строка j столбец
     */
...
// содержится ли значение переменной в масиве
    ...
    /**
     * строкиа на основе матрици 1 2 3 4 5 6 2;3;1;2;3;4;5;6
     */
...
// !??????????????!переделать
    public boolean equals(Object anObject) {
        if (anObject instanceof Matrix) {
            ...
            for (int i = 1; i <= m; i++) {
                for (int j = 1; j <= n; j++) {
                    if (!getElement(i, j).equals(A.getElement(i, j))) {
                        b = false;
                    }
                }
            }
    ...
    public static Matrix calcGaus(Matrix A, Matrix B) {
        Matrix result = new Matrix(A.getM(), A.getN());
        int cRow = 0;
        for (int i = 1; i <= B.getM(); i++) {
            if (A.getElement(i, 1).equals(new Q("1"))) {
                cRow = i;
                break;
            }
        }
        if (cRow == 0) {
            for (int i = 1; i <= B.getM(); i++) {
                if (!A.getElement(i, 1).equals(new Q("0"))) {
                    cRow = i;
                    break;
                }
